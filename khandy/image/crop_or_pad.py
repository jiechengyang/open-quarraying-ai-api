...
def crop(image, x_min, y_min, x_max, y_max, border_value=0):
    ...
    assert isinstance(x_min, numbers.Integral) and isinstance(
        y_min, numbers.Integral)
    assert isinstance(x_max, numbers.Integral) and isinstance(
        y_max, numbers.Integral)
...
def crop_or_pad_coords(boxes, image_width, image_height):
    warnings.warn(
        'crop_or_pad_coords will be deprecated, use crop_coords instead!')
...
def center_crop(image, dst_width, dst_height, strict=True):
    ...
    """
    assert khandy.is_numpy_image(image)
    assert isinstance(dst_width, numbers.Integral) and isinstance(
        dst_height, numbers.Integral)
    src_height, src_width = image.shape[:2]
    if strict:
